...
import { renderer, scene, camera } from "./js/InitRenderer.js";
/**
 * UdeM IFT 3355, A25
 * TP 1 Template
 */
...
Promise.all(shaderFiles.map(
  path => new Promise(resolve => loader.load(path, source => resolve({ path, source })))
)).then(results => {
  // Turn into a dictionary like SourceLoader used to
  const shaders = {};
  results.forEach(r => shaders[r.path] = r.source);

  // Assign to your materials
...
function updateScene() {
	...
	armadilloMaterial.uniforms[ 'time' ].value = (Math.floor(Date.now() / 1000));
}
